...
New-ADUser -SamAccountName $login -Name $login -DisplayName ($user.FirstName + " " + $user.LastName) -Surname $user.LastName -UserPrincipalName ($login + $dnsroot) -Description $user.title -Enabled $true -ChangePasswordAtLogon $false -PasswordNeverExpires $true -AccountPassword $defpassword -PassThru 
